...
> **L4** and **L9** — H1, H2, H3, M1–M6, L1 and L9 across commits `ce038d1` …
> `6875d83`, and L4 since. Closed rows are struck through and annotated inline;
> L2, L3 and L5–L8 and the Info rows stand as audited.
>
...
| **L4** | Licensing         | ~~Low~~ **CLOSED**    | ~~Licence state is evaluated once at startup and cached for the process lifetime; expiry mid-session takes effect only on restart~~ — fixed: a detached watcher re-runs `evaluate_license` every 15 min and pushes `license://changed`, the app's first backend-emitted event, so the gate _and_ the screen follow within a tick | Done on a timer, not on focus — focus is the renderer's signal, and the gate that matters is in Rust. The location hint was stale: the startup call is `lib.rs:163`, not `:81` | `commands.rs` (`start_license_watcher`, `publish_license`); `lib.rs:178`; `license.rs:315-357` |
...
One residual: **L2** (dev key only warns in a release binary). **L4** — evaluated
once at startup — is closed: the verdict is re-evaluated every 15 minutes while
the app runs, so an expiry no longer waits for a restart.
